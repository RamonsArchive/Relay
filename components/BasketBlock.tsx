...
const BasketBlock = ({userId, item, cartId, temp_cartId}: {userId: string, item: BasketType, cartId: number, temp_cartId: string | null}) => {
  ...
  return (
    <div className={`flex flex-row gap-y-2 w-full h-[200px] xs:h-[250px] gap-x-5 justify-start pt-5 border-b-[1px] bg-gray-50 border-gray-300 pb-5 pt-5 first:pt-3 last:border-b-0`}>
            ...
            <div className="flex flex-col w-full gap-y-3">
                ...
                <div className="flex flex-row gap-x-5 items-center">
                    <p className="font-plex-sans text-[18px] xs:text-[20px] md:text-[22px] text-gray-500 font-bold">
                        ${lineSubtotal ? (lineSubtotal / 100).toFixed(2) : ((price || 0) / 100).toFixed(2)}
                    </p>
